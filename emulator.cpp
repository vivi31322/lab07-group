...
int parse_instr(int line, char* ftok, instr* imem, int memoff, label_loc* labels, source* src) {
	...
	if ( pscnt > 0 ) {
		...
		switch( op ) {
			...
			//instruction added
			case MUL:case MULH:case MULHSU:case MULHU:case DIV:case DIVU:case REM:case REMU:case ANDN:
			case CLMUL:case CLMULH:case CLMULR:case MAX:case MAXU:case MIN:case MINU:
...
void execute(uint8_t* mem, instr* imem, label_loc* labels, int label_count, bool start_immediate) {
	...
	while(!dexit) {
		...
		switch (i.op) {
			...
			case DIV:rf[i.a1.reg] = (int32_t)rf[i.a2.reg] / (int32_t)rf[i.a3.reg]; break;
			case DIVU:rf[i.a1.reg] = (uint32_t)rf[i.a2.reg] / (uint32_t)rf[i.a3.reg]; break;
			...
			case CLMUL:
				rf[i.a1.reg] = 0;
				for (int j = 0; j < 32; j++) {
					if ((rf[i.a3.reg] >>(uint32_t) j)&(uint32_t)1)
						rf[i.a1.reg] = rf[i.a1.reg] ^ (rf[i.a2.reg] << (uint32_t)j);
				}
				break;
			case CLMULH:
				rf[i.a1.reg] = 0;
				for (int j = 0; j < 32; j++) {
					if ((rf[i.a3.reg] >> j)&(uint32_t)1)
